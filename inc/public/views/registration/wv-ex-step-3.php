...
$field_of_work = $global_user_data['wv_fieldOfWork'] ?? '';
$selected_category = $profile_selection;
if ($field_of_work == 'Spirits' || $field_of_work == 'Food') {
    $selected_category = $field_of_work;
}
...
<div class="wv-step" id="<?php echo esc_attr($step_key); ?>">
    ...
    <div id="wv-step-body" class="py-32 px-24 px-lg-128">
        ...
        <div class="row justify-content-start align-items-stretch g-12">
            <?php foreach ($categories as $cat):
                $checked = (!empty($saved_data['wv_userCategory']) && $saved_data['wv_userCategory'] === $cat) ? 'checked' : '';
            ?>
            <?php if ($selected_category == 'Spirits') : ?>
            ...
            <?php endif; ?>
                <label class="wv-custom-radio">
                    <input
                        ...
                        <?php echo $checked; ?>
                    >
                    <div class="wv-radio-card wv-radio-card-inline">
                        <div class="wv-check">
                            <span class="wv wv_check-50"><span class="path1"></span><span class="path2"></span></span>
                        </div>
                        <h3 class="h2 ps-8"><?php echo esc_html($cat); ?></h3>
                    </div>
                </label>
            </div>
            <?php endforeach; ?>
